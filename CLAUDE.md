...
- エラー記録と通知: `src/lib/log.ts`(`logSystem` で `system_logs` に記録。error は設定画面の通知先メールと `ALERT_WEBHOOK_URL` へ通知、同じ source は 30 分に 1 回)。`src/instrumentation.ts` の `onRequestError` でサーバー側の未捕捉エラーを自動記録(Cookie から操作ユーザーとテナントを調べて通知の件名・本文に入れる。`src/lib/supabase/request-context.ts`、セッションのリフレッシュはしない)。通知先はテナントごとの設定画面「通知先メールアドレス」(`app_settings.alert_emails`)で、システム全体のログは運営側テナントの設定を使う。`/api/health` は外形監視用(認証なしは DB のみ、cron/ログイン時は IMAP/SMTP も確認)。ログは `/settings/logs`。
