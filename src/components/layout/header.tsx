...
export function Header({ user, tenantName }: { user: { email: string | null; full_name: string | null }; tenantName?: string }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center justify-end gap-3 border-b bg-background/80 backdrop-blur px-4 md:px-6">
      {tenantName && <span className="mr-auto truncate text-sm font-medium text-muted-foreground">{tenantName}</span>}
      <MailSyncButton />
      <DropdownMenu>
