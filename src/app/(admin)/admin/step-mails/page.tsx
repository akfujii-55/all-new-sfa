import { listStepMails } from "@/actions/step-mails";
...
export default async function StepMailsPage() {
  const steps = await listStepMails();
  return (
    <div className="max-w-4xl">
      <PageHeader
        title="ステップメール"
        description="お試し中の顧客(テナントの連絡先)へ、テナント作成からの日数に合わせて自動で送るメール。毎朝の定期処理(7:00 ごろ)で、運営側の既定のメールアカウントから送ります。"
      />
