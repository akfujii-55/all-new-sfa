...
export default async function AppLayout({ children }: LayoutProps<"/">) {
  ...
  const [{ data: profile }, { data: member }, { count: unread }, { count: inquiries }] = await Promise.all([
  ...
  const signature = buildSignature(member?.name || profile?.full_name || (auth.user.email ?? "").split("@")[0]);
