...
async function requireTenant() {
  ...
  if (!tenant) throw userError("所属する会社(テナント)を確認できません");
...
export async function startCheckout(): Promise<never> {
  ...
  try {
    ...
    throw userError("決済ページを開けませんでした。時間をおいてお試しいただくか、運営にお問い合わせください");
  }
  redirect(url);
}
...
export async function openBillingPortal(): Promise<never> {
  if (!stripeConfigured()) throw userError("オンライン決済は準備中です。運営にお問い合わせください");
  const { supabase, tenant, user } = await requireTenant();
  if (!tenant.stripe_customer_id) throw userError("まだ契約手続きが行われていません");
  let url: string;
  try {
    ...
    throw userError("お支払い管理ページを開けませんでした。時間をおいてお試しいただくか、運営にお問い合わせください");
  }
  redirect(url);
}
