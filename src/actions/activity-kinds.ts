...
function parse(formData: FormData) {
  ...
  if (!name) throw new Error("種類の名前を入力してください");
  if (name.length > 30) throw new Error("種類の名前は 30 文字以内にしてください");
  if (!ACTIVITY_ICONS.some((i) => i.key === icon)) throw new Error("アイコンの指定が不正です");
  return { name, icon };
}
...
export async function createActivityKind(formData: FormData) {
  ...
  if ((count ?? 0) >= MAX_ACTIVITY_KINDS) throw new Error(`行動の種類は ${MAX_ACTIVITY_KINDS} 個までです`);
  const { error } = await supabase.from("activity_kinds").insert({ name, icon, sort_order: count ?? 0 });
  if (error) {
    if (error.code === "23505") throw new Error("同じ名前の種類があります");
...
export async function deleteActivityKind(id: string) {
  ...
  if ((used ?? 0) > 0) throw new Error(`この種類を使っている行動が ${used} 件あるため削除できません。名前を変えて使うか、行動の種類を変更してください`);
  if ((total ?? 0) <= 1) throw new Error("行動の種類は最低 1 個必要です");
  const { error } = await supabase.from("activity_kinds").delete().eq("id", id);
  if (error) {
    if (error.code === "23503") throw new Error("この種類を使っている行動があるため削除できません");
    throw new Error(error.message);
  }
  revalidateKindPages();
}
